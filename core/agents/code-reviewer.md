...
- **Docs updated in the same PR:** any discovery that extends or contradicts a
  ...
  deferred.
- **Scope discipline:** the diff builds only the slice that was asked for — no
  drive-by refactors or unrequested features folded in; deferrals and follow-ups
  are listed explicitly, not silently expanded or silently dropped.
