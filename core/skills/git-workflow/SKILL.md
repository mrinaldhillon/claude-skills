...
The portable trunk-based baseline. To bind it to a repo, **add a companion — don't copy it
...
- **Companion** `git-workflow-<yourrepo>` fills the slots — the concrete **branch-routing
  ...
  and a `CLAUDE.md` pin are the two supported extension surfaces; forking this file is neither.)
- **Decisions of record** (protection posture, routing) live in that repo's `CLAUDE.md` or a
  decisions doc the companion points to — this skill stays repo-agnostic.
