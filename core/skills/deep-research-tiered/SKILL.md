...
the load-bearing facts are inlined here.
...
multi-agent costs ~15× single-agent chat (see Grounding); spend that only on high-value,
...
Opus-1M and poured full web pages into a 1M context. Tier by stakes:
...
that degrade coherence (see Grounding). Parallelize *investigation* (search, fetch,
...
## Grounding
The load-bearing figures are inlined above; the primary sources behind them:
...
- **Context is finite, with diminishing returns (context rot)** — spend the right
  context per prompt.
  (anthropic.com/engineering/effective-context-engineering-for-ai-agents)
- **Single-thread the coupled write.** (cognition.com/blog/dont-build-multi-agents)
- **Verification uses independent critics**, not orchestrator-as-judge.
  (anthropic.com/research/building-effective-agents)
- The per-stage tiering mandate cross-references the `orchestration` skill ›
  *Workflow fan-out: tier every stage, guard the budget, checkpoint before loss*.
- The ~1.95M-token burn and the worker-Write failure counts (6/18 to CWD, 12/18
  nowhere) with the `journal.jsonl` recovery recipe come from the maintainer's own
  audit records; journal path/schema re-verified on disk 2026-07-16.
