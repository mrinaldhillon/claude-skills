...
| hook    | `block-main-writes`    | PreToolUse(Bash): denies `git commit`/`merge`/`push` when the repo the command **targets** is on `main` (trunk-based backstop). The target is the invocation's own literal `git -C <path>`, else the chain's `cd <path>`, else the session's `cwd` — so a commit from a worktree on a branch is allowed even while the anchor checkout sits on `main`, and a `git -C <main-repo> push` is denied even while the session sits on a branch. Project-scoped here so trunk-PR discipline is **opt-in per repo**, not global — moved out of `core` for exactly that reason. |
