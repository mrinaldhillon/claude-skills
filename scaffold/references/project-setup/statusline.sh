...
# context-nudge hook (shipped by the scaffold plugin) can read. Hook payloads
# carry no token counts; the status line JSON is the only live source of
# context_window.used_percentage. The bridge also records session_id — which
# session's percentage this is — so the nudge hook can stay silent in sessions
# the bridge does not describe.
set -euo pipefail
