...
# `claude plugin validate` and the checks below are complementary, not redundant:
# it reads the plugin.json schema (field types, unknown fields, dependency shape)
# which we only `jq empty`; we cover marketplace wiring, component frontmatter and
# hook scripts, which it does not inspect. Run both wherever the CLI exists.
#
# Skipped LOUDLY, not silently, when Claude Code is absent — the CI image has no
# `claude`, so this degrades to the hand-rolled checks there rather than failing.
claude_bin="$(command -v claude 2>/dev/null || true)"
[ -n "$claude_bin" ] || note 'claude CLI not on PATH — skipping "claude plugin validate" schema checks'
...
validate_plugin() {
  ...
  # Valid JSON is not a valid manifest: `jq empty` above accepts any well-formed
  # document, so a wrong-typed field ships clean. --strict also promotes warnings
  # (unrecognized fields) to errors so typos cannot land silently.
  if [ -n "$claude_bin" ]; then
    if out="$("$claude_bin" plugin validate "$dir" --strict 2>&1)"; then
      pass "plugin '$name': claude plugin validate --strict"
    else
      err "plugin '$name': claude plugin validate --strict reported:"
      printf '%s\n' "$out" | sed 's/^/        /' >&2
    fi
  fi
