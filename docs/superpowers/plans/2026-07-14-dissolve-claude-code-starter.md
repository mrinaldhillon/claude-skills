...
> **⚠️ REVISION 2026-07-14 — the generator is DROPPED (user + advisor).** Tasks 1 & 2 stand as written and are done. **Task 3 is reshaped** (Task 3R): its bundled content moves from a generator `templates/` tree into documented `scaffold/references/project-setup/` (examples + a setup README), and the `CLAUDE.md` template is dropped. **Tasks 4 and 5 are DROPPED** (no `emit-substrate`/`bootstrap` skill/command, no CI self-test; no `adopt` script/skill/command). **Task 6** now tests the `references/project-setup/` example scripts. **Task 7** documents "enable the plugins + follow `references/project-setup/`" instead of a bootstrap flow. `/scaffold:adopt` becomes a documented migration checklist inside that README. Rationale: a plugin that copies plugin-coupled files (settings.json, CLAUDE.md) into projects owns a permanent drift liability — the opposite of the clean, best-practice bundling the user asked for. The paragraphs below describing the generator are retained for provenance but are superseded by this banner.
