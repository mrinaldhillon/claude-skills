...
> **⚠️ REVISION 2026-07-14 — the GENERATOR is DROPPED (user + advisor, during implementation).** The design below (a `/scaffold:bootstrap` generator that *emits* substrate, `emit-substrate.sh`, bundled `templates/`, `/scaffold:adopt`) is **superseded**. A plugin that copies *plugin-coupled* files (settings.json, CLAUDE.md) into projects owns a permanent drift liability — the opposite of the clean, best-practice bundling that is the actual goal (surfaced concretely when the emitted `CLAUDE.md` immediately went stale against the plugin's own hooks).
>
> **Revised design (what ships):** `core` + `scaffold` are correctly-bundled plugins with clean deps (`scaffold`→`core`). All logic lives in them; the milestone runner lives in `scaffold/scripts/` behind `/scaffold:milestone-run`. The project-local substrate is **not generated** — it ships as **documented references** under `scaffold/references/`: the full machinery ADRs in `decisions/`, and `project-setup/` holding `settings.example.json` (the complement), the `statusline.sh`+`context-nudge.sh` example scripts, the `.context/` convention, the ADR baseline (seeds + stubs + TEMPLATE), and a **`README.md`** that preserves the correctness knowledge in prose — the settings **complement + double-fire hazard**, branch-before-first-commit, and the migration checklist (the former `/scaffold:adopt`, now a hand checklist). A new project **enables the plugins and follows that README by hand**; zero dependency on the `claude-code-starter` *repo* still holds (conventions live in the plugin as reference; the project owns its own files). **Kept from the original design:** the 21 logic components already in the plugins, the runner relocation, the hook tests migrated into CI, and the by-nature ADR classification. The sections below are retained for provenance; read them through this banner.
...
2. **`scaffold`** (per-repo). Gains:
   ...
   - **plugin reference docs**: the *machinery-rationale* ADRs `0003` (checkpoint-resume mechanism) + `0004` (milestone loop) + `0005` (agent-state layout — cited by `0003`/`0004` as their authority-of-record, so it lives with the machinery) + `0006` (permission profiles);
   - the **migrated tests** (§10).
3. **Generated project substrate** (per bootstrapped repo, emitted by the generator): `.context/` seed + `README.md`; `milestones/example.json`; `docs/decisions/` seed ADRs — the *convention* ADRs `0000,0001,0002,0007` plus pointer stubs at `0003,0004,0005,0006` (§8) — and `TEMPLATE.md`; `docs/` skeletons; filled `CLAUDE.md`; `.claude/statusline.sh` + `.claude/context-nudge.sh`; and `.claude/settings.json` **complement** wiring (§6).
...
- **ADR split — by *nature*, not by number** (classification confirmed against the ADR bodies, 2026-07-14). Classify each as *project convention / decision-of-record* (seed) vs *plugin-machinery rationale* (plugin reference):
  - **Seed (conventions every project needs):** `0000` record-decisions, `0001` trunk-based, `0002` branch-protection-by-discipline, `0007` annotate-superseded-in-place (**amends `0000` — must travel with it**, else a dangling amendment pointer).
  - **Plugin reference (machinery rationale that `/plugin update` owns):** `0003` context-mgmt-by-checkpoint-resume, `0004` context-managed milestone loop, `0005` agent-state-in-`.context/`, `0006` milestone permission profiles. `0003`'s Decision section is machinery — it names `statusline.sh`/`context-nudge.sh`/`checkpoint.sh`/the runner and the 55%/65% thresholds — so seeding it verbatim would desync on `/plugin update`; the checkpoint-resume *convention* is instead carried by the project's `CLAUDE.md` §"Context & checkpoint protocol". **`0005` is machinery too** — the config-audit found `0003`/`0004` cite it as their superseding authority-of-record, so it must live in the machinery collection (`references/decisions/`) to keep that trail self-contained; the practical `.context/` layout is still carried for the project by `context/README.md`.
  - **Numbering rule (append-only preserved):** the four relocated slots are seeded as **one-paragraph pointer stubs** at `0003`/`0004`/`0005`/`0006` ("machinery of record now lives in the scaffold plugin's ADR set; slot retained to preserve append-only numbering"). This keeps the project's decision log contiguous with **no gaps and no desync** (the stub is stable; the full rationale evolves in the plugin). The project's `/adr` continues from `0008`.
