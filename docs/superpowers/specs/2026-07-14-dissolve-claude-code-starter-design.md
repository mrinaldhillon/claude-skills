...
2. **`scaffold`** (per-repo). Gains:
   ...
   - **plugin reference docs**: the *machinery-rationale* ADRs `0003` (checkpoint-resume mechanism) + `0004` (milestone loop) + `0006` (permission profiles);
   - the **migrated tests** (§10).
3. **Generated project substrate** (per bootstrapped repo, emitted by the generator): `.context/` seed + `README.md`; `milestones/example.json`; `docs/decisions/` seed ADRs — the *convention* ADRs `0000,0001,0002,0005,0007` plus pointer stubs at `0003,0004,0006` (§8) — and `TEMPLATE.md`; `docs/` skeletons; filled `CLAUDE.md`; `.claude/statusline.sh` + `.claude/context-nudge.sh`; and `.claude/settings.json` **complement** wiring (§6).
...
- **ADR split — by *nature*, not by number** (classification confirmed against the ADR bodies, 2026-07-14). Classify each as *project convention / decision-of-record* (seed) vs *plugin-machinery rationale* (plugin reference):
  - **Seed (conventions every project needs):** `0000` record-decisions, `0001` trunk-based, `0002` branch-protection-by-discipline, `0005` agent-state-in-`.context/` (defines the `.context/` layout the generator *creates*; its taxonomy is restated in the generated `.context/README.md`), `0007` annotate-superseded-in-place (**amends `0000` — must travel with it**, else a dangling amendment pointer).
  - **Plugin reference (machinery rationale that `/plugin update` owns):** `0003` context-mgmt-by-checkpoint-resume, `0004` context-managed milestone loop, `0006` milestone permission profiles. `0003`'s Decision section is machinery — it names `statusline.sh`/`context-nudge.sh`/`checkpoint.sh`/the runner and the 55%/65% thresholds — so seeding it verbatim would desync on `/plugin update`; the checkpoint-resume *convention* is instead carried by the generated `CLAUDE.md` §"Context & checkpoint protocol".
  - **Numbering rule (append-only preserved):** the three relocated slots are seeded as **one-paragraph pointer stubs** at `0003`/`0004`/`0006` ("machinery of record now lives in the scaffold plugin's ADR set; slot retained to preserve append-only numbering"). This keeps the project's decision log contiguous with **no gaps and no desync** (the stub is stable; the full rationale evolves in the plugin). Seed `0005`'s cross-references to `0003 §4` / `0004 §1` resolve through those stubs. The project's `/adr` continues from `0008`.
